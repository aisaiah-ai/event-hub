...
export const e = functions.runWith({ invoker: "public" }).https.onRequest(async (req, res) => {
  if (req.method !== "GET") {
  ...
  try {
    ...
    if (shortlinkSnap.exists) {
      ...
      eventId = (data?.eventId as string) || rawId;
    }
  } catch {
  ...
  try {
    ...
    if (eventSnap.exists) {
      ...
      title = (data.title as string) || (data.name as string) || title;
      venue = (data.venue as string) || "";
      description = (data.description as string) || (data.subtitle as string) || "";
