...
function makeSnapshot(exists, data = {}, id = "doc-id") {
    return {
        exists: exists,
        id,
        data: () => data,
...
function makeDocRef(path) {
    const ref = {
        ...
        collection: (id) => makeCollectionRef(`${path}/${id}`),
        path,
    };
    return ref;
}
...
jest.mock("firebase-admin", () => {
    const firestoreInstance = {
        collection: (id) => makeCollectionRef(id),
        runTransaction: (fn) => mockTransaction(fn),
    };
...
describe("API", () => {
    ...
    describe("GET /", () => {
        it("returns discovery JSON with ok and v1 endpoints", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/");
    ...
    describe("GET /v1/events", () => {
        it("returns 200 and array (empty when no events)", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events");
            expect(res.status).toBe(200);
    ...
    describe("GET /v1/events/:eventId", () => {
        it("returns 404 when event does not exist", async () => {
            mockDocGet.mockReturnValue(makeSnapshot(false));
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nonexistent");
        ...
        it("returns 200 and event when exists", async () => {
            ...
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nlc-2026");
            expect(res.status).toBe(200);
            expect(res.body.ok).toBe(true);
    ...
    describe("GET /v1/events/:eventId/sessions", () => {
        it("returns 404 when event does not exist", async () => {
            mockDocGet.mockReturnValue(makeSnapshot(false));
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nonexistent/sessions");
            expect(res.status).toBe(404);
        ...
        it("returns 200 and array when event exists", async () => {
            mockDocGet.mockReturnValue(makeSnapshot(true, {}));
            mockQueryGet.mockReturnValue(makeQuerySnapshot([]));
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nlc-2026/sessions");
            expect(res.status).toBe(200);
            expect(res.body).toEqual({ ok: true, data: [] });
        });
    });
    describe("GET /v1/events/:eventId/announcements", () => {
        it("returns 404 when event does not exist", async () => {
            mockDocGet.mockReturnValue(makeSnapshot(false));
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nonexistent/announcements");
            expect(res.status).toBe(404);
        });
        it("returns 200 and array when event exists", async () => {
            mockDocGet.mockReturnValue(makeSnapshot(true, {}));
            mockCollectionGet.mockReturnValue(makeQuerySnapshot([]));
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nlc-2026/announcements");
            expect(res.status).toBe(200);
            expect(res.body).toEqual({ ok: true, data: [] });
        });
    });
    ...
    describe("Auth-required routes", () => {
        ...
        it("POST /v1/events/:eventId/register returns 401 with invalid token", async () => {
            ...
            if (res.status === 500 && ((_b = (_a = res.body) === null || _a === void 0 ? void 0 : _a.error) === null || _b === void 0 ? void 0 : _b.code) === "internal") {
                expect(res.body.error.message).toBeDefined();
            }
        });
        it("GET /v1/me/registrations returns 401 without token", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/me/registrations");
            expect(res.status).toBe(401);
            expect(res.body.error.code).toBe("unauthenticated");
        });
        it("GET /v1/events/:eventId/my-registration returns 401 without token", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nlc-2026/my-registration");
            expect(res.status).toBe(401);
        });
        it("POST /v1/events/:eventId/checkin/main returns 401 without token", async () => {
            const res = await (0, supertest_1.default)(api_1.default).post("/v1/events/nlc-2026/checkin/main").send({});
            expect(res.status).toBe(401);
        });
        ...
        it("GET /v1/events/:eventId/checkin/status returns 401 without token", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/events/nlc-2026/checkin/status");
            expect(res.status).toBe(401);
        });
    });
    ...
    describe("404", () => {
        it("returns 404 and error shape for unknown path", async () => {
            const res = await (0, supertest_1.default)(api_1.default).get("/v1/unknown");
            expect(res.status).toBe(404);
